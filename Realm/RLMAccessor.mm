...
id makeSetter(__unsafe_unretained RLMProperty *const prop) {
    ...
    return ^(__unsafe_unretained RLMObjectBase *const obj, ArgType val) {
        ...
        RLMObservationTracker tracker(obj->_observationInfo, obj->_row.get_key(), *obj->_info);
        tracker.willChange(name);
...
void RLMAccessorContext::will_change(realm::Obj const& row, realm::Property const& prop) {
    if (!_observationHelper) {
        auto info = RLMGetObservationInfo(nullptr, row.get_key(), _info);
        if (info || prop.type == realm::PropertyType::Object) {
            _observationHelper = std::make_unique<RLMObservationTracker>(info, row.get_key(), _info);
        }
    }
    if (_observationHelper) {
        _observationHelper->willChange(_info.propertyForTableColumn(prop.column_key).name);
