...
public:
    ...
    RLMObservationTracker(RLMObservationInfo *info, realm::ObjKey row, RLMClassInfo& objectSchema);
    ~RLMObservationTracker();

    bool hasDirectObservers() const noexcept { return _info; }
    void trackDeletions();

    void willChange(NSString *key, NSKeyValueChange kind=NSKeyValueChangeSetting, NSIndexSet *indexes=nil);
    void didChange();
