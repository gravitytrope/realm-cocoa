...
RLMObservationTracker::RLMObservationTracker(RLMObservationInfo *info, realm::ObjKey row, RLMClassInfo& objectSchema)
: RLMObservationTracker(objectSchema.realm)
{
    _info = RLMGetObservationInfo(info, row, objectSchema);
}
...
void RLMObservationTracker::willChange(NSString *key, NSKeyValueChange kind, NSIndexSet *indexes) {
    _key = key;
    _kind = kind;
    _indexes = indexes;
    if (_info) {
